...
import { selectedEvents, setSelectedEvents, triggerUpdate, setTriggerSelection, setHasSelection, setSelection, isEventHidden, selectedRange } from "../../../store/selectionStore";
...
export default function DataTable_Small(props: { context?: string; useSelectionIndexColors?: boolean }) {
  // Get context from props, default to 'dataset' for backward compatibility
  const context = props?.context || 'dataset';
  const useSelectionIndexColors = props?.useSelectionIndexColors === true;
  ...
  // Watch for selectedEvents changes (from cross-window sync) and update table
  createEffect(() => {
  ...
  const renderRows = createMemo(() => {
    ...
    return rowsToRender.map((row) => {
      ...
      const isColoredBySource = color() === 'SOURCE' && !useSelectionIndexColors;
      
      let rowColor = "white"; // Default to white
      
      if (isSelected) {
        if (useSelectionIndexColors && !hasMoreThan8Selections) {
          // VIDEO view: use selection-index colors (s10/s20) so table rows match video tiles
          rowColor = getIndexColor(selected, row.event_id) ?? "white";
      ...
      return (
        ...
        >
          {columns().map((column, colIndex) => {
            ...
            // Get color for categorical columns (SOURCE, MAINSAIL, HEADSAIL, RACE, STATE, CONFIG, YEAR, EVENT)
            let cellStyle = {};
            if (column === 'SOURCE' || column === 'MAINSAIL' || column === 'HEADSAIL' || column === 'RACE' || column === 'STATE' || column === 'CONFIG' || column === 'YEAR' || column === 'EVENT') {
