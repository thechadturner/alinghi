...
interface PolarRoseBuilderProps {
    objectName?: string;
    [key: string]: any;
}

export default function PolarRoseBuilder(props: PolarRoseBuilderProps) {
    const navigate = useNavigate();

    const chartType = 'POLAR ROSE';
    ...
    // Polar Rose specific settings
    const [xAxisIntervals, setXAxisIntervals] = createSignal(36); // Number of directional bins
    const [yAxisIntervals, setYAxisIntervals] = createSignal(1); // Wind speed bin size in knots
    const [colorScheme, setColorScheme] = createSignal("warm-to-cold"); // Default color scheme

    // Color scheme options for polar rose
    ...
    // Initialize with a single polar rose chart
    const initializePolarRose = (): void => {
        if (chartObjects.length === 0) {
            const polarRoseChart = {
                unique_id: generateUniqueId(),
                series: [
                    {
                        xaxis: { name: "twd", type: "float", interval: xAxisIntervals() }, // Angular channel
                        yaxis: { name: "tws", type: "float", interval: yAxisIntervals() }, // Magnitude channel
                        data: [],
                    },
                ],
                filters: [],
            ...
            setChartObjects([polarRoseChart]);
        }
    };

    // Update polar rose settings
    const updatePolarRoseSettings = (settings) => {
        if (chartObjects.length > 0) {
    ...
    const fetchCharts = async () => {
        ...
        try {
            const response_json = await getData(`${apiEndpoints.app.users}/object?class_name=${encodeURIComponent(selectedClassName())}&project_id=${encodeURIComponent(selectedProjectId())}&user_id=${encodeURIComponent(user().user_id)}&parent_name=polarrose&object_name=${chartObjectName()}`, controller.signal);
            ...
            if (response_json.success) {
                try {
                    ...
                    if (chartData && chartData.length > 0) {
                        ...
                        try {
                            const namesUrl = `${apiEndpoints.app.users}/object/names?class_name=${encodeURIComponent(selectedClassName())}&project_id=${encodeURIComponent(selectedProjectId())}&user_id=${encodeURIComponent(user()!.user_id)}&parent_name=polarrose`;
                        ...
                        if (chartsWithIds[0] && chartsWithIds[0].series && chartsWithIds[0].series[0]) {
                            setXAxisIntervals(chartsWithIds[0].series[0].xaxis?.interval || 36);
                            setYAxisIntervals(chartsWithIds[0].series[0].yaxis?.interval || 1);
    ...
    const saveCharts = async (saveAsName?: string) => {
        ...
        if (isNewChartPlaceholderName(objectName)) {
            warn('PolarRose builder: Cannot save until the chart name is updated from "new chart".');
        ...
        try {
            ...
            if (nameAlreadyExists && !isUpdatingOwnChart) {
                warn('PolarRose builder: Save blocked - chart name already exists:', objectName);
            ...
            const chartObject = {"parent_name": "polarrose", "chart_name": "default", "chart_info": updatedCharts, "shared": sharedFlag()};
            const summary_json = {'class_name': selectedClassName(), 'project_id': selectedProjectId(),'user_id': user().user_id, 'parent_name': 'polarrose', 'object_name': objectName, 'json': JSON.stringify(chartObject, null, 2)};
    ...
    const saveAsNewChart = async () => {
        ...
        if (newName && newName.trim()) {
            ...
            if (isNewChartPlaceholderName(trimmed)) {
                warn('PolarRose builder: Cannot use "new chart" as the chart name.');
    ...
    onMount(async () => {
        await fetchCharts();
        // Initialize polar rose if no charts exist
        if (chartObjects.length === 0) {
            initializePolarRose();
        }
        setLoading(false);
                ...
                <div class="builder-page-header">
                    <div class="builder-page-title">
                        <div class="builder-page-title-content">
                            <h1>Polar Rose Chart Builder</h1>
                            <p>Create and configure interactive polar rose plot visualizations</p>
                        </div>
                    </div>
                </div>
                <form class="builder-form">
                            ...
                            <div class="flex flex-wrap items-start gap-6">
                                <div class="flex-1 min-w-[400px]">
                                    ...
                                    <input
                                        ...
                                        class={`builder-form-input w-full px-4 py-2 text-sm select-text ${(isNewChartPlaceholderName(chartObjectName()) || chartNameError() === 'duplicate') ? 'builder-form-input-invalid' : ''}`}
                    ...
                    <Show when={chartObjects.length > 0} fallback={
                        <div class="text-center py-16">
                            ...
                            <h3 class="text-xl font-semibold mb-2" style="color: var(--color-text-primary);">Initializing Polar Rose Chart</h3>
                            <p class="mb-8 max-w-md mx-auto" style="color: var(--color-text-secondary);">Setting up your wind analysis chart...</p>
                        </div>
                    }>
                        <div class="builder-chart-container">
                            <For each={chartObjects}>
                                {(chart, chartIndex) => (
                                    <div class="builder-form-card">
                                        {/* Polar Rose Chart Header */}
                                        <div class="builder-form-header">
                                            <div class="flex items-center space-x-3">
                                                <div class="w-10 h-10 rounded-lg flex items-center justify-center" style="background: var(--color-bg-button);">
                                                    <svg class="w-6 h-6" style="color: var(--color-text-inverse);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                                    </svg>
                                                </div>
                                                <div>
                                                    <h3 class="text-lg font-semibold" style="color: var(--color-text-primary);">Polar Rose Chart</h3>
                                                    <p class="text-sm" style="color: var(--color-text-secondary);">Wind direction and speed analysis</p>
                                                </div>
                                            </div>
                                        </div>
                                        
                                        {/* Polar Rose Configuration */}
                                        <div class="builder-form-content">
                                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                                {/* Data Channels */}
                                                <div class="space-y-4">
                                                    <h4 class="text-sm font-semibold uppercase tracking-wide" style="color: var(--color-text-primary);">Data Channels</h4>
                                                    
                                                    <div class="space-y-3">
                                                        <div>
                                                            <label class="block text-sm font-medium mb-2" style="color: var(--color-text-primary);">Angular Channel</label>
                                                            <input
                                                                type="text"
                                                                value={chart.series?.[0]?.xaxis?.name || ""}
                                                                placeholder="Click to select angular channel"
                                                                onClick={() => {
                                                        ...
                                                        <div>
                                                            <label class="block text-sm font-medium mb-2" style="color: var(--color-text-primary);">Magnitude Channel</label>
                                                            <input
                                                                type="text"
                                                                value={chart.series?.[0]?.yaxis?.name || ""}
                                                                placeholder="Click to select magnitude channel"
                                                                onClick={() => {
                                                                    setSelectedChart(chartIndex);
                                                                    setShowChannels(true);
                                                                    setShowAxis("yaxis");
                                                ...
                                                {/* Chart Settings */}
                                                <div class="space-y-4">
                                                    <h4 class="text-sm font-semibold uppercase tracking-wide" style="color: var(--color-text-primary);">Chart Settings</h4>
                                                    
                                                    <div class="space-y-3">
                                                        <div>
                                                            <label class="block text-sm font-medium mb-2" style="color: var(--color-text-primary);">Angular Intervals (Directional Bins)</label>
                                                            <select
                                                                value={xAxisIntervals()}
                                                                onChange={(e) => {
                                                                    setXAxisIntervals(parseInt(e.target.value));
                                                                    updatePolarRoseSettings({ xAxisIntervals: parseInt(e.target.value) });
                                                                }}
                                                                class="builder-form-input w-full px-3 py-2 text-sm"
                                                            >
                                                                <option value={16}>16 bins (22.5° each)</option>
                                                                <option value={36}>36 bins (10° each)</option>
                                                                <option value={72}>72 bins (5° each)</option>
                                                            </select>
                                                        </div>
                                                        
                                                        <div>
                                                            <label class="block text-sm font-medium mb-2" style="color: var(--color-text-primary);">Magnitude Intervals (Value Bins)</label>
                                                            <select
                                                                value={yAxisIntervals()}
                                                                onChange={(e) => {
                                                                    setYAxisIntervals(parseInt(e.target.value));
                                                                    updatePolarRoseSettings({ yAxisIntervals: parseInt(e.target.value) });
                                                                }}
                                                                class="builder-form-input w-full px-3 py-2 text-sm"
                                                            >
                                                                <option value={1}>1 unit increments</option>
                                                                <option value={2}>2 unit increments</option>
                                                                <option value={5}>5 unit increments</option>
                                                                <option value={10}>10 unit increments</option>
                                                            </select>
                                                        </div>
                                                        
                                                        <div>
                                                            <label class="block text-sm font-medium mb-2" style="color: var(--color-text-primary);">Color Scheme</label>
                                                            <select
                                                                value={colorScheme()}
                                                                onChange={(e) => {
                                                                    setColorScheme(e.target.value);
                                                                    updatePolarRoseSettings({ colorScheme: e.target.value });
                                                                }}
                                                                class="builder-form-input w-full px-3 py-2 text-sm"
                                                            >
                                            ...
                                            {/* Filters Section */}
                                            <div class="mt-6 pt-6" style="border-top: 1px solid var(--color-border-primary);">
                                                <h4 class="text-sm font-semibold uppercase tracking-wide mb-3" style="color: var(--color-text-primary);">Data Filters</h4>
