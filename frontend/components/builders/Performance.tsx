...
import TargetPicker from "../utilities/TargetPicker"; // Import TargetPicker
...
export default function PerformanceBuilder(props: PerformanceBuilderProps) {
    const navigate = useNavigate();

    const chartType = 'PERFORMANCE';
    ...
    const [showTargetChannels, setShowTargetChannels] = createSignal(false);
    ...
    // Update group additional text
    const updateGroupAdditionalText = (groupIndex, newText) => {
        setGroupObjects(groupIndex, "additionalText", newText);
        setHasChanges(true);
    };
    ...
    const fetchCharts = async () => {
        ...
        try {
            ...
            const response_json = await getData(`${apiEndpoints.app.users}/object?class_name=${encodeURIComponent(selectedClassName())}&project_id=${encodeURIComponent(selectedProjectId())}&user_id=${encodeURIComponent(user().user_id)}&parent_name=performance&object_name=${chartObjectName()}&page_name=performance`, controller.signal);

            if (response_json.success) {
                try {
                    const chartInfo = response_json.data?.chart_info || [];
                    const normalizedChartInfo = chartInfo.map((group: any) => ({
                        ...group,
                        charts: group.charts?.map((chart: any) => ({
                            ...chart,
                            series: chart.series?.map((series: any) => {
                                const oldAggregate = series.aggregate || series.yaxis?.aggregate || series.type || "AVG";
                                const { aggregate, ...seriesWithoutAggregate } = series;
                                return {
                                    ...seriesWithoutAggregate,
                                    xType: series.xType || "AVG",
                                    yType: series.yType || oldAggregate,
                                    xaxis: series.xaxis || { name: "DEFAULT", type: "float" },
                    ...
                    setGroupObjects(normalizedChartInfo);
                    const loadedName = normalizedChartInfo.length > 0 ? chartObjectName() : null;
                    setLoadedChartObjectName(loadedName);
                    if (loadedName !== null) {
                        try {
                            const namesUrl = `${apiEndpoints.app.users}/object/names?class_name=${encodeURIComponent(selectedClassName())}&project_id=${encodeURIComponent(selectedProjectId())}&user_id=${encodeURIComponent(user()!.user_id)}&parent_name=performance`;
    ...
    const saveCharts = async (saveAsName?: string) => {
        ...
        if (isNewChartPlaceholderName(objectName)) {
            warn('Performance builder: Cannot save until the chart name is updated from "new chart".');
        ...
        try {
            ...
            if (nameAlreadyExists && !isUpdatingOwnChart) {
                warn('Performance builder: Save blocked - chart name already exists:', objectName);
            ...
            const cleanedGroupObjects = groupObjects.map((group: any) => ({
                ...group,
                charts: group.charts?.map((chart: any) => ({
                    ...chart,
                    series: chart.series?.map((series: any) => {
                        const { aggregate, ...seriesWithoutAggregate } = series;
                        return seriesWithoutAggregate;
                    }) || []
                })) || []
            }));
            const chartObject = {"parent_name": "performance", "chart_name": "default", "chart_info": cleanedGroupObjects, "shared": sharedFlag()};
            const summary_json = {'class_name': selectedClassName(), 'project_id': selectedProjectId(), 'user_id': user().user_id, 'parent_name': 'performance', 'object_name': objectName, 'json': JSON.stringify(chartObject, null, 2)};
            log("Saving performance object:", JSON.stringify(groupObjects, null, 2));
    ...
    const saveAsNewChart = async () => {
        const newName = prompt("Enter a new name for this performance chart:", `${chartObjectName()} Copy`);
        if (newName && newName.trim()) {
            const trimmed = newName.trim();
            if (isNewChartPlaceholderName(trimmed)) {
                warn('Performance builder: Cannot use "new chart" as the chart name.');
            ...
            if (axis === "yaxisEdit") {
                ...
                setGroupObjects(groupIndex, "charts", 0, "series", (series = []) => {
                    ...
                    if (selectedIndex >= 0 && selectedIndex < updatedSeries.length) {
                        const existingSeries = updatedSeries[selectedIndex];
                        updatedSeries[selectedIndex] = {
                            ...
                            xType: existingSeries.xType || "AVG",
                            yType: existingSeries.yType || "AVG"
                        };
                    }
                    return updatedSeries;
                });
            } else if (axis === "xaxisEdit") {
                // Update the x-axis channel for the selected series when edited via picker
                const channel = channels[0];
                setGroupObjects(groupIndex, "charts", 0, "series", (series = []) => {
                    const updatedSeries = [...series];
                    const selectedIndex = selectedSeries();
                        ...
                        if (axis === "yaxis") {
                            ...
                            return [
                                ...series,
                                {
                                    ...
                                    xaxis: { name: "DEFAULT", type: "float" }, // Default x-axis
                                    yaxisTarget: { name: "", type: "float" }, // Default empty target
                                    xType: "AVG", // Default x-type
                                    yType: "AVG", // Default y-type
                                    info_type: "info",
                                    info_message: "",
                                },
                            ];
                        } else if (axis === "yaxisTarget") {
                            // Update the target channel (yaxisTarget) for the selected series
                            const updatedSeries = [...series];
                            const selectedIndex = selectedSeries();
                            if (selectedIndex >= 0 && selectedIndex < updatedSeries.length) {
                                updatedSeries[selectedIndex] = {
                                    ...updatedSeries[selectedIndex],
                                    yaxisTarget: { name: channel, type: "float" },
                                };
                            }
                            return updatedSeries;
                        } else if (axis === "xaxis") {
                            // Update the x-axis channel for the selected series
                            const updatedSeries = [...series];
                            const selectedIndex = selectedSeries();
                            if (selectedIndex >= 0 && selectedIndex < updatedSeries.length) {
                                updatedSeries[selectedIndex] = {
                                    ...updatedSeries[selectedIndex],
                                    xaxis: { name: channel, type: "float" },
                                };
                            }
                            return updatedSeries;
                        }
    ...
    const handleUpdateTargetName = (groupIndex, seriesIndex, newName) => {
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "yaxisTarget", "name", newName);
        setHasChanges(true);
    };

    const handleUpdateXAxisName = (groupIndex, seriesIndex, newName) => {
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "xaxis", "name", newName);
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "xaxis", "type", "float");
        setHasChanges(true);
    };

    const handleUpdateXType = (groupIndex, seriesIndex, newType) => {
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "xType", newType);
        setHasChanges(true);
    };

    const handleUpdateYType = (groupIndex, seriesIndex, newType) => {
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "yType", newType);
        // Also remove old aggregate field if it exists
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, (series) => {
            const { aggregate, ...rest } = series;
            return { ...rest, yType: newType };
        });
        setHasChanges(true);
    };

    const handleUpdateInfoType = (groupIndex, seriesIndex, newType: string) => {
        setGroupObjects(groupIndex, "charts", 0, "series", seriesIndex, "info_type", newType);
        setHasChanges(true);
    };

    const handleUpdateInfoMessage = (groupIndex, seriesIndex, newMessage: string) => {
    ...
    const handleDrop = (e: DragEvent, groupIndex: () => number, seriesIndex: () => number) => {
        ...
        const series = group?.charts?.[0]?.series;
        if (!series) return;
        const newSeries = reorderSeries(series, draggedIdx, seriesIdx);
    ...
    return (
        <div class="builder-page builder-page-performance">
            {loading() && <Loading />}
                ...
                <div class="builder-page-header">
                    <div class="builder-page-title">
                        <div class="builder-page-title-content">
                            <h1>Performance Page Builder</h1>
                            <p>Create and manage performance charts for data visualization</p>
                        </div>
                    </div>
                </div>
                <form class="builder-form">
                    ...
                    <Show when={groupObjects.length > 0} fallback={
                        <div class="text-center py-16">
                            ...
                            <h3 class="text-xl font-semibold mb-2" style="color: var(--color-text-primary);">The page is empty</h3>
                            <p class="mb-8 max-w-md mx-auto" style="color: var(--color-text-secondary);">Would you like to add charts?</p>
                            <button
                                type="button"
                                onClick={(e) => {
                                    e.preventDefault();
                                ...
                                {(group, groupIndex) => (
                                    <div class="builder-form-card">
                                                ...
                                                <div class="flex items-center space-x-3">
                                                    ...
                                                    <div class="flex-1 flex items-center space-x-2">
                                                        <input
                                                            ...
                                                            class="builder-form-input flex-1"
                                                        />
                                                        <input
                                                            type="text"
                                                            value={group.additionalText || ""}
                                                            placeholder="Additional text (optional)"
                                                            onInput={(e) => updateGroupAdditionalText(groupIndex(), e.target.value)}
                                                            class="builder-form-input flex-1"
                                                        />
                                                    </div>
                                                </div>
                                        ...
                                        <div class="p-4">
                                            ...
                                            <div class="mb-6">
                                                    <h4 style="color: var(--color-text-primary);">Channels & Targets</h4>
                                                    <div class="overflow-hidden border border-gray-200 rounded-lg">
                                                        <table class="builder-table builder-table-fixed min-w-full" style={{ "table-layout": "fixed", width: "100%" }}>
                                                            <thead>
                                                                <tr>
                                                                    <th class="w-10"></th>
                                                                    <th style={{ width: "16%", "min-width": "16%", "max-width": "16%" }}>X-Axis</th>
                                                                    <th style={{ width: "10%", "min-width": "10%", "max-width": "10%" }}>X-Type</th>
                                                                    <th style={{ width: "18%", "min-width": "18%", "max-width": "18%" }}>Y-Axis</th>
                                                                    <th style={{ width: "10%", "min-width": "10%", "max-width": "10%" }}>Y-Type</th>
                                                                    <th style={{ width: "18%", "min-width": "18%", "max-width": "18%" }}>Target</th>
                                                                    <th style={{ width: "10%", "min-width": "10%", "max-width": "10%" }}>Info Type</th>
                                                                    <th style={{ width: "12%", "min-width": "12%", "max-width": "12%" }}>Info Msg</th>
                                                                    <th style={{ width: "6%", "min-width": "6%", "max-width": "6%" }}>Actions</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                            <For each={group.charts[0]?.series || []}>
                                                                {(seriesItem, seriesIndex) => (
                                                                    ...
                                                                    >
                                                                            <td class="builder-drag-handle-cell px-3 py-3">
                                                                                <DragHandleIcon />
                                                                            </td>
                                                                            <td class="px-3 py-3" style={{ width: "16%", "min-width": "16%", "max-width": "16%" }}>
                                                                                <input
                                                                                    type="text"
                                                                                    value={seriesItem?.xaxis?.name || "DEFAULT"}
                                                                                    placeholder="DEFAULT"
                                                                                    onInput={(e) => {
                                                                                        handleUpdateXAxisName(groupIndex(), seriesIndex(), e.target.value);
                                                                                    }}
                                                                                    onClick={() => {
                                                                                        setSelectedGroup(groupIndex()); // Set the selected group
                                                                                        setSelectedSeries(seriesIndex()); // Set the selected series
                                                                                        setShowChannels(true); // Show the ChannelPicker
                                                                                        setShowAxis("xaxisEdit"); // Set axis to edit x-axis
                                                                                    }}
                                                                                    class="builder-form-input cursor-pointer w-full"
                                                                                />
                                                                            </td>
                                                                            <td class="px-3 py-3" style={{ width: "10%", "min-width": "10%", "max-width": "10%" }}>
                                                                                <select
                                                                                    value={seriesItem?.xType || "AVG"}
                                                                                    onChange={(e) => {
                                                                                        handleUpdateXType(groupIndex(), seriesIndex(), e.target.value);
                                                                                    }}
                                                                            ...
                                                                            <td class="px-3 py-3" style={{ width: "18%", "min-width": "18%", "max-width": "18%" }}>
                                                                                <input
                                                                                    type="text"
                                                                            ...
                                                                            <td class="px-3 py-3" style={{ width: "10%", "min-width": "10%", "max-width": "10%" }}>
                                                                                <select
                                                                                    value={seriesItem?.yType || "AVG"}
                                                                                    onChange={(e) => {
                                                                                        handleUpdateYType(groupIndex(), seriesIndex(), e.target.value);
                                                                                    }}
                                                                                    class="builder-form-input w-full"
                                                                                >
                                                                                    <option value="AVG">AVG</option>
                                                                            ...
                                                                            <td class="px-3 py-3" style={{ width: "18%", "min-width": "18%", "max-width": "18%" }}>
                                                                                <input
                                                                                    type="text"
                                                                                    value={seriesItem?.yaxisTarget?.name || ""}
                                                                                    placeholder="Select Target"
                                                                                    onInput={(e) => {
                                                                                        handleUpdateTargetName(groupIndex(), seriesIndex(), e.target.value);
                                                                                    }}
                                                                                    onClick={() => {
                                                                                        setSelectedGroup(groupIndex()); // Set the selected group
                                                                                        setSelectedSeries(seriesIndex()); // Set the selected series
                                                                                        setShowTargetChannels(true); // Show the ChannelPicker
                                                                                        setShowAxis("yaxisTarget"); // Set axis to target
                                                                                    }}
                                                                                    class="builder-form-input cursor-pointer w-full"
                                                                                />
                                                                            </td>
                                                                            ...
                                                                            <td class="px-3 py-3" style={{ width: "12%", "min-width": "12%", "max-width": "12%" }}>
                                                                                <input
                                                                                    type="text"
                                                                            ...
                                                                            <td class="px-3 py-3 text-center" style={{ width: "6%", "min-width": "6%", "max-width": "6%" }}>
                                                                            <svg
                    ...
                    {/* Save / Save As - show Save when hasChanges; disable when non-owner or placeholder name */}
