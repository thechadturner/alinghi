...
import { error as logError, warn } from "../../utils/console";
...
interface ProbabilityBuilderProps {
    objectName?: string;
    isFleet?: boolean;
    type?: string;
    [key: string]: any;
...
export default function ProbabilityBuilder(props: ProbabilityBuilderProps) {
    const navigate = useNavigate();

    const chartType = 'PROBABILITY';
    ...
    // Fleet mode: use fleet_probability parent when in day/fleet mode
    ...
    const parentName = createMemo(() => (isFleet() ? 'fleet_probability' : 'probability'));
    ...
    const chartTypeOptions: string[] = ["Histogram", "Standard Probability", "Categorical Probability"];
    const displayTypeOptions: string[] = ["Standard", "Cumulative"];
    ...
    const addChart = (): void => {
        const newChart = {
            ...
            series: [
                {
                    xaxis: { name: "" }, // Single X-axis entry
                    yaxis: { 
                        name: "Standard Probability", // Chart type replaces yaxis name
                        cumulative: false // Cumulative flag in yaxis
                    },
                    color: d3.schemeCategory10[chartObjects.length % 10], // Default color
                    data: [],
                },
            ],
    ...
    const saveCharts = async (saveAsName?: string) => {
        ...
        if (isNewChartPlaceholderName(objectName)) {
            warn('Probability builder: Cannot save until the chart name is updated from "new chart".');
        ...
        try {
            ...
            if (nameAlreadyExists && !isUpdatingOwnChart) {
                warn('Probability builder: Save blocked - chart name already exists:', objectName);
            ...
            const chartObject = {"parent_name": parentName(), "chart_name": "default", "chart_info": chartObjects, "shared": sharedFlag(), "columns": columns()};
    ...
    const saveAsNewChart = async () => {
        ...
        if (newName && newName.trim()) {
            ...
            if (isNewChartPlaceholderName(trimmed)) {
                warn('Probability builder: Cannot use "new chart" as the chart name.');
    ...
    const deleteChart = async () => {
        ...
        try {
            ...
            if (response_json.success) {
                setSidebarMenuRefreshTrigger(1);
                unifiedDataStore.clearAllData(); // Clear the data store after deleting
    ...
    const handleChartTypeChange = (chartIndex, chartType) => {
        setChartObjects(chartIndex, "series", 0, "yaxis", "name", chartType);
        setHasChanges(true);
    };

    const handleDisplayTypeChange = (chartIndex, cumulative) => {
        setChartObjects(chartIndex, "series", 0, "yaxis", "cumulative", cumulative === "Cumulative");
        setHasChanges(true);
    };
    ...
    const handleSaveChannels = async (axis, channels) => {
        ...
        if (channels.length > 0) {
            // Only allow the first channel for probability charts
            // Preserve original case - Probability.tsx handles case-insensitive matching
            // This ensures InfluxDB channels (which may have mixed case) work correctly
                ...
                <div class="builder-page-header">
                    <div class="builder-page-title">
                        <div class="builder-page-title-content">
                            <h1>Probability Chart Builder</h1>
                            <p>Create and configure interactive probability visualizations</p>
                        </div>
                    </div>
                </div>
                <form class="builder-form">
                    ...
                    <Show when={chartObjects.length > 0} fallback={
                        <div class="text-center py-16">
                            ...
                            <p class="mb-8 max-w-md mx-auto" style="color: var(--color-text-secondary);">Create your first probability chart to start visualizing your data</p>
                            <button
                                type="button"
                                onClick={(e) => {
                                    e.preventDefault();
                                            ...
                                            <div class="space-y-3">
                                                ...
                                                <div class="overflow-x-auto">
                                                    <table class="builder-table min-w-full" style="table-layout: fixed; width: 100%;">
                                                        <thead>
                                                            <tr>
                                                                <th style="width: 25%;">X-Axis</th>
                                                                <th style="width: 25%;">Chart Type</th>
                                                                <th style="width: 25%;">Display Type</th>
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                ...
                                                                <td style="width: 25%;">
                                                                    <select
                                                                        value={chart.series?.[0]?.yaxis?.name || "Standard Probability"}
                                                                ...
                                                                <td style="width: 25%;">
                                                                    <select
                                                                        value={chart.series?.[0]?.yaxis?.cumulative ? "Cumulative" : "Standard"}
                                                                        onChange={(e) => handleDisplayTypeChange(chartIndex(), e.target.value)}
                                                                        class="builder-form-input w-full px-3 py-2 text-sm"
                                                                    >
                                                ...
                                                {/* Filters Section */}
                                                <div>
                                                    <h4 class="text-sm font-semibold uppercase tracking-wide mb-3" style="color: var(--color-text-primary);">Chart Filters</h4>
