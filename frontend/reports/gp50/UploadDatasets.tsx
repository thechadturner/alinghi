...
export default function UploadDatasetsPage() {
  ...
  const handleFileChange = async (event: Event) => {
    ...
    // Filter for .xml and _10HZ.csv files only
    const allowedFiles = selectedFiles.filter(file => {
      const fileNameLower = file.name.toLowerCase();
      return fileNameLower.endsWith('.xml') || fileNameLower.endsWith('_10hz.csv');
    });
    
    if (allowedFiles.length === 0) {
      debug('[UploadDatasets] No .xml or _10HZ.csv files selected');
      return;
    }
  ...
  const handleFolderChange = async (event: Event) => {
    ...
    if (allowedFiles.length === 0) {
      debug('[UploadDatasets] No .xml or _10HZ.csv files found in selected folder');
      return;
    }
        ...
        for (const [sourceId, dates] of datesBySourceArray.entries()) {
          ...
          for (let i = 0; i < uniqueSourceDates.length; i++) {
            ...
            const datasetPayload = {
                ...
                tags: JSON.stringify({'isUploaded': true})
            };
          ...
          <div class="form-group">
            ...
            <div class="input-container">
              ...
              <select
                ...
                class="form-input"
                style="appearance: auto; padding-right: 40px;"
              >
              ...
              <Show when={!batchMode()}>
                <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
                  ...
                  <label for="folderMode" style="cursor: pointer; color: var(--color-text-primary); font-size: 14px;">
                    Select folder (finds all .xml and _10HZ.csv files)
                  </label>
                </div>
              </Show>
              ...
              <Show when={!folderMode()}>
                <div class="file-upload-container">
                  ...
                  <label for="fileInput" class="file-upload-label">
                    ...
                    <span class="file-upload-subtext">XML files and _10HZ.csv files</span>
                  </label>
                </div>
              </Show>
              ...
              <Show when={folderMode()}>
                <div class="file-upload-container">
                  ...
                  <label for="folderInput" class="file-upload-label">
                    ...
                    <span class="file-upload-subtext">All .xml and _10HZ.csv files in folder will be selected</span>
                  </label>
                </div>
              </Show>
            </div>
              ...
              <div style="display: flex; align-items: center; gap: 12px;">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Clear source name when switching to batch mode
                    if (e.target.checked) {
                      setSourceName("");
                      // Automatically enable folder mode when batch mode is enabled
                      setFolderMode(true);
                  ...
                  style="width: 18px; height: 18px; cursor: pointer;"
                />
                <label for="batchMode" style="cursor: pointer; color: var(--color-text-primary); font-size: 14px;">
                  Batch mode (match files to sources by filename)
                </label>
              </div>
              <p style="margin-top: 8px; margin-left: 30px; color: var(--color-text-secondary); font-size: 12px;">
                {batchMode() 
                  ? "Files will be automatically matched to sources based on filename. Leave unchecked to use a single source for all files."
                  : "All files will be uploaded to the specified source. Check to enable batch mode and match files to sources by filename."}
              </p>
              
              <div style="display: flex; align-items: center; gap: 12px; margin-top: 16px;">
                <input
                  type="checkbox"
                  id="onlyCsvMatchingXmlFolders"
                  checked={onlyCsvMatchingXmlFolders()}
                  onChange={(e) => {
                    setOnlyCsvMatchingXmlFolders(e.target.checked);
                    // If enabling filter, re-filter existing files
                    if (e.target.checked && files().length > 0) {
                      const { filteredFiles, xmlFolders, csvFilteredCount } = filterCsvByXmlFolders(files());
                      setFiles(filteredFiles);
                      if (xmlFolders.size === 0) {
                        toastStore.showToast('info', 'CSV Filter Applied', 'No XML files found - only XML files will be uploaded (no CSV files)');
                      } else if (csvFilteredCount > 0) {
                        toastStore.showToast('info', 'CSV Filter Applied', `${csvFilteredCount} CSV file(s) filtered - only folders with XML files included (${xmlFolders.size} folder(s) found)`);
                      }
                    }
                  }}
                  style="width: 18px; height: 18px; cursor: pointer;"
                />
                <label for="onlyCsvMatchingXmlFolders" style="cursor: pointer; color: var(--color-text-primary); font-size: 14px;">
                  Only upload CSV files matching XML folder names
                </label>
              </div>
              <p style="margin-top: 8px; margin-left: 30px; color: var(--color-text-secondary); font-size: 12px;">
                When checked, only CSV files in folders containing XML files will be uploaded. First finds all XML files and their folder names, then filters CSV files to match those folders.
              </p>
            </div>
          </Show>
          
          <button type="submit" class="login-button" disabled={files().length === 0 || (!batchMode() && !sourceName()) || !eventName()}>
