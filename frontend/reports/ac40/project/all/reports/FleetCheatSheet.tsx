...
export default function FleetCheatSheetPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (sources.length > 0) {
        url += `&source_names=${encodeURIComponent(JSON.stringify(sources))}`;
      }
      ...
      setRows(normalized);
      logDebug("FleetCheatSheet: fetched rows", { count: normalized.length, groupBy: group });
    } catch (err) {
      logError("FleetCheatSheet: fetch failed", err);
  ...
  const fetchManeuverData = async () => {
    ...
    try {
      ...
      logDebug("FleetCheatSheet Maneuvers: fetched rows", { count: normalized.length, groupBy: group });
    } catch (err) {
      logError("FleetCheatSheet Maneuvers: fetch failed", err);
  ...
  onMount(() => {
    logPageLoad("FleetCheatSheet.tsx", "Fleet Cheat Sheet", "Loaded");
    const cleanupScaling = setupMediaContainerScaling({
      logPrefix: "FleetCheatSheet",
      scaleToWidth: true,
    });
  ...
  const maneuverDisplay = (): string => maneuverType().toUpperCase();
  ...
  const copyTableToClipboard = async () => {
    try {
      ...
      log("FleetCheatSheet: copy table to clipboard", { textLength: clipboardText.length });
      ...
      logError("FleetCheatSheet: copy table failed", err);
    }
  };
  ...
  const copyTableToClipboardManeuver = async () => {
    try {
      ...
      log("FleetCheatSheet Maneuvers: copy table to clipboard", { textLength: clipboardText.length });
      if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(clipboardText);
      ...
      logError("FleetCheatSheet Maneuvers: copy table failed", err);
    }
  };
        ...
        <div class="performance-charts-scroll-container">
          <div class="performance-charts-scroll-inner cheat-sheet-scroll-inner">
            <Show when={sourcesStore.isReady() && sourcesStore.sources().length > 0}>
              <div class="fleet-cheat-sheet-sources-block">
                <div class="fleet-cheat-sheet-sources-header">
                  <span class="fleet-cheat-sheet-sources-label">Selected sources</span>
                  <div class="fleet-cheat-sheet-sources-actions">
                    <button
                      type="button"
                      class="fleet-cheat-sheet-sources-action-btn"
                      onClick={() => {
                        const allNames = sourcesStore.sources().map((s) => s.source_name).filter(Boolean);
                        setSelectedSources([...allNames]);
                        logDebug("FleetCheatSheet: Select All sources", { count: allNames.length });
                      }}
                    >
                      Select All
                    </button>
                    <button
                      ...
                      onClick={() => {
                        setSelectedSources([]);
                        logDebug("FleetCheatSheet: None - cleared selected sources");
                      }}
                    >
                      None
                    </button>
                ...
                <div class="fleet-cheat-sheet-sources-row">
                  <For each={[...sourcesStore.sources()].sort((a, b) => a.source_id - b.source_id)}>
                    {(source) => {
                      const name = source.source_name;
                      const isSelected = () => selectedSources().includes(name);
                      const sourceColor = source.color ?? sourcesStore.getSourceColor(name) ?? getSourceFallbackColor(name);
                      return (
                        <button
                          type="button"
                          class={`fleet-cheat-sheet-source-pill px-3 py-1 rounded-full text-sm cursor-pointer font-medium transition-colors ${
                            isSelected() ? "fleet-cheat-sheet-source-pill-selected" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                          }`}
                          style={{ ["--fleet-pill-bg" as string]: sourceColor }}
                          onClick={() => {
                            ...
                            logDebug("FleetCheatSheet: source toggled", { name, selected: next.includes(name), totalSelected: next.length });
                          }}
                        >
                          {name}
                        </button>
            ...
            <h2 class="report-page-title cheat-sheet-title">Straight Line</h2>
            ...
            <div class="race-summary-dropdown-section cheat-sheet-dropdowns">
              <DropDownButton
                ...
                handleSelection={async (item) => {
                  ...
                  await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet", `Group by changed to ${item}`);
                }}
              />
              <Show when={groupBy() === "channel"}>
                <DropDownButton
                  ...
                  handleSelection={async (item) => {
                    ...
                    const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                    await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet", `TWS changed to ${item}`);
                  }}
                />
              </Show>
              <Show when={groupBy() === "wind"}>
                <DropDownButton
                  ...
                  handleSelection={async (item) => {
                    ...
                    const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                    await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet", `Metric changed to ${item}`);
                  }}
                />
              </Show>
              <DropDownButton
                ...
                handleSelection={async (item) => {
                  ...
                  setLegType(leg);
                  const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                  await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet", `Point of sail changed to ${item}`);
                }}
              />
            </div>

            <Show when={loading()}>
              <div class="cheat-sheet-table-loading-wrapper">
                    ...
                    <label class="race-summary-deltas-checkbox">
                      <input
                        ...
                        onChange={async (e) => {
                          ...
                          const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                          await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet", `Show deltas ${checked ? "on" : "off"}`);
                        }}
                      />
                ...
                <div class="race-summary-dropdown-section cheat-sheet-dropdowns">
                  <DropDownButton
                    ...
                    handleSelection={async (item) => {
                      ...
                      const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                      await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet Maneuvers", `Group by changed to ${item}`);
                    }}
                  />
                  <Show when={groupByManeuver() === "channel"}>
                    <DropDownButton
                      ...
                      handleSelection={async (item) => {
                        ...
                        const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                        await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet Maneuvers", `TWS changed to ${item}`);
                      }}
                    />
                  </Show>
                  <Show when={groupByManeuver() === "wind"}>
                    <DropDownButton
                      ...
                      handleSelection={async (item) => {
                        ...
                        const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                        await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet Maneuvers", `Metric changed to ${item}`);
                      }}
                    />
                  </Show>
                  <DropDownButton
                    ...
                    handleSelection={async (item) => {
                      ...
                      setManeuverType(m);
                      const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                      await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet Maneuvers", `Maneuver type changed to ${item}`);
                    }}
                  />
                </div>
                          ...
                          <label class="race-summary-deltas-checkbox">
                            <input
                              ...
                              onChange={async (e) => {
                                ...
                                const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
                                await logActivity(project_id, dataset_id, "FleetCheatSheet.tsx", "Fleet Cheat Sheet Maneuvers", `Show deltas ${checked ? "on" : "off"}`);
                              }}
                            />
                            <span>Show deltas</span>
                          </label>
