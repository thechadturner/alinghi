...
export default function UploadDatasetsPage() {
  ...
  const handleUploadWithProcessDecision = async (cancelRunning: boolean) => {
    ...
    try {
      // Training Day: ensure dataset row per session date, then upload `.db` to data/raw (no normalization / Python here)
      if (isTrainingDay()) {
        ...
        for (const yyyymmdd of rTrain.dates) {
          ...
          await ensureDatasetRowForUploadDate({
            sourceName: src,
            yyyymmdd,
            timezone: timezone(),
            className: selectedClassName(),
        ...
        setCurrentStatus('All database files uploaded to data/raw and converted to parquet under data/system.');
        ...
        toastStore.showToast(
          'success',
          'Upload complete',
          `${dbFiles.length} file(s) saved under data/raw; parquet written for session date(s) ${trainDateLabel} and source.`
        );
      ...
      toastStore.showToast(
        'success',
        'Upload complete',
        `${jsonlFiles.length} file(s) saved under data/raw for session date(s) ${raceDateLabel} (JSONL).`
      );
      setUploadSuccess(true);
      setShowWaiting(false);
    } catch (error: unknown) {
