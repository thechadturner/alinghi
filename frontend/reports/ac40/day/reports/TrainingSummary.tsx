...
/** Row from race-summary API (data.js): speed fields use server-side physical units; keys from `raceApiFieldNames`. */
...
/** Row from race-setup API: VMG field key from `RaceSetupApiKeys`. */
...
export default function TrainingSummaryPage() {
  ...
  /** When true, races list is from TRAINING hours (HOUR 0, 1, 2, ...); show "Hour N" in UI and pass hour index to APIs */
  const [isTrainingHourMode, setIsTrainingHourMode] = createSignal(false);
  ...
  const [legType, setLegType] = createSignal<string>("upwind");
  ...
  const raceSummaryTableColumns = createMemo(() =>
    buildRaceSummaryTableColumns(persistantStore.defaultUnits()) as { key: keyof RaceSummaryRow; header: string }[]
  );
  ...
  /** Dropdown options for race/hour selector. Training mode: show "11:00", "12:00" etc.; race mode: show race numbers. */
  const raceDropdownOptions = (): string[] => {
    const list = races();
  ...
  /** Resolve dropdown display label to race/hour key for API. Training mode: key is the label (e.g. "11:00"). */
  const raceDisplayToKey = (display: string): string => {
    if (display === "All") return "All";
    if (isTrainingHourMode()) return display;
    if (display.startsWith("Hour ")) return display.slice(5).trim();
    return display;
  };

  /** Current race/hour key as dropdown display label. Training mode: show "11:00", "12:00" (no "Hour " prefix). */
  const selectedRaceDisplayLabel = (): string => {
    const r = selectedRace();
    if (r === "All") return "All";
    if (isTrainingHourMode()) return r;
    return r;
  };
  ...
  const fetchRaces = async () => {
    ...
    try {
      ...
      const rows = result.data as { Race_number?: number; HOUR?: number }[] | null | undefined;
      if (!Array.isArray(rows)) {
        setRaces([]);
        setIsTrainingHourMode(false);
        return;
      }
      const hasHour = rows.some((r) => r?.HOUR !== undefined && r?.HOUR !== null);
      if (hasHour) {
        const hourKeys = rows
  ...
  /** Fetch race summary from race-summary endpoint. Training Summary uses summary_type=training; when raceKey is "All" send race=0 for aggregate across all training hours. */
  const fetchRaceSummary = async (raceKey: string) => {
    ...
    try {
      const raceParam = raceKey === "All" ? "0" : raceKey;
      const url = `${apiEndpoints.app.data}/race-summary?class_name=${encodeURIComponent(className)}&project_id=${encodeURIComponent(projectId)}&date=${encodeURIComponent(date)}&race=${encodeURIComponent(raceParam)}&summary_type=training`;
      const result = await getData(url);
      if (!result?.success) {
        setSummaryRows([]);
        return;
  ...
  const fetchRaceSetup = async () => {
    ...
    try {
      let url = `${apiEndpoints.app.data}/race-setup?class_name=${encodeURIComponent(className)}&project_id=${encodeURIComponent(projectId)}&date=${encodeURIComponent(date)}&leg_type=${encodeURIComponent(legType())}`;
    ...
    try {
      ...
      for (const row of rowsList) {
        const cells = summaryCols.map((col) => {
          ...
          const toDisplay = col.key === "distance_m" ? num / 1000 : num;
          return formatNum(toDisplay);
        });
    ...
    <div id="media-container" class="race-summary-page">
      ...
      <Show when={!loading() && !error() && !hasNoData()}>
        ...
        <div class="race-summary-dropdown-section" style={{ "margin-bottom": "1.5rem" }}>
          <RaceSettings />
          <DropDownButton
            options={raceDropdownOptions()}
            defaultText={selectedRaceDisplayLabel()}
            smallLabel={isTrainingHourMode() ? "Hour" : "Races"}
            size="big"
            handleSelection={async (item) => {
              const key = raceDisplayToKey(item);
              setSelectedRace(key);
              const { project_id, dataset_id } = await getCurrentProjectDatasetIds();
              await logActivity(project_id, dataset_id, "RaceSummary.tsx", "Race Summary", isTrainingHourMode() ? `Hour changed to ${key}` : `Race changed to ${key}`);
            }}
          />
        </div>
                            ...
                            <tr>
                              <For each={raceSummaryTableColumns()}>
                                {(col) => {
                                  ...
                                  const isDistanceKm = keyStr === "distance_m";
                                  const toDisplay = (v: number) => (isDistanceKm ? v / 1000 : v);
                                  const delta =
                                    showDelta &&
                                    val != null &&
                                    firstVal != null &&
                                    ...
                                    !Number.isNaN(Number(firstVal))
                                      ? toDisplay(Number(val)) - toDisplay(Number(firstVal))
                                      : null;
                                  const displayText = val != null ? formatNum(toDisplay(Number(val))) : "—";
                                  const deltaText = delta != null ? formatDelta(delta) : null;
                                  return (
                                    <td
                                      class={`centered ${scaleClass}`}
                                    >
                                      {displayText}
                                      {deltaText != null && (
                        ...
                        <tr>
                          <td colSpan={raceSummaryTableColumns().length} class="centered" style={{ padding: "1rem", color: "var(--color-text-secondary)" }}>
                            {selectedRace() === "All"
                              ...
                              : isTrainingHourMode()
                                ? "No race-summary data for this hour. Ensure data/race-summary has data for the selected hour."
        ...
        <section style={{ "margin-bottom": "1rem" }}>
          ...
          <h2 class="report-section-heading" style={{ "font-size": "1rem", "font-weight": "600", "margin-bottom": "0.5rem" }}>
            {selectedRace() === "All"
              ? "Average Setup:"
              : isTrainingHourMode()
                ? `Hour ${selectedRace()} Setup:`
                : `Race ${selectedRace()} Setup:`}
          </h2>
          <div
            class="copy-table-hover-wrapper"
            ...
            <Show
              ...
              fallback={
                <p class="race-summary-empty-message">
                  No averages data for this date and leg type. Check that race-setup data exists for the selected REACH / UPWIND / DOWNWIND.
                </p>
              }
            >
              <div class="scrollable-container">
          ...
          {averagesRows().length > 0 && averagesTableEmptyMetrics() && (
            <p style={{ "color": "var(--color-text-secondary)", "font-size": "0.875rem", "margin-top": "0.5rem" }}>
              No metric values for this leg type — ensure <code>race_stats</code> has data for the selected date{isTrainingHourMode() ? "/hour" : "/race"} and REACH / UPWIND / DOWNWIND.
            </p>
          )}
        </section>
      </Show>
