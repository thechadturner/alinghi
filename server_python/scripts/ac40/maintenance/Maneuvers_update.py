"""
Maneuvers_update.py - Update maneuver statistics for existing TACK and GYBE events

OVERVIEW:
This script updates maneuver statistics for existing TACK and GYBE events 
in the database. It loops through multiple date/source combinations, retrieves data,
and computes maneuver statistics for existing events without creating new ones.
...
- Computes maneuver statistics for TACK and GYBE events
- Updates maneuver_stats values in database via: PUT /api/events/row
- Supports updating missing data columns for existing maneuver events
...
3. PUT /api/events/row - Updates individual maneuver_stats column values
   - Updates specific columns for existing maneuver events
...
def get_data(api_token, project_id, class_name, date, source_name, channels, start_ts, end_ts, verbose):
    ...
    try:
        ...
        u.log(api_token, "Maneuvers_update.py", "error", "get_data", str(e))
        return df

#COMPUTE STATS 
def computeStats(api_token, verbose, class_name, project_id, dataset_id, event_type, df):
    if verbose:
        print('Computing maneuver stats: '+str(event_type), flush=True)  

    u.log(api_token, "Maneuvers_update.py", "info", "computing maneuver stats", str(event_type))
            ...
            for index, period in enumerate(json_data): 
                ...
                if start_ts is not None and end_ts is not None and not pd.isna(start_ts) and not pd.isna(end_ts):
                    ...
                    if isinstance(dff, pd.DataFrame):
                        if len(dff) > 0:                     
                            twa = dff['Twa_n_deg'].mean()
                            if twa < 90:
                                drop_tgt = dff['ANGLE_CANT_DROP_TARG_UW_deg'].mean()
                            else:
                                drop_tgt = dff['ANGLE_CANT_DROP_TARG_DW_deg'].mean()
                            
                            # Update DB_cant_drop_tgt_deg column in maneuver_stats
                            data = {
                                "class_name": str(class_name),
                                "project_id": int(project_id),
                                "table": "maneuver_stats",
                                "event_id": int(event_id),
                                "agr_type": "NONE",
                                "column": "Cant_drop_tgt",
                                "value": str(round(drop_tgt, 3))
                            }
                            ...
                            if res.get("success"):
                                ...
                                if verbose and update_count % 10 == 0:
                                    print(f"Maneuver stats updated {update_count} events...", flush=True)
                            else:
                                if verbose:
                                    if not res.get("success"):
                                        print(f"Maneuver stats update failed for event {event_id} (DB_cant_drop_tgt_deg): {res}", flush=True)
            
            if verbose and update_count > 0:
                print(f"Maneuver stats completed: {update_count} events updated!", flush=True)
        else:
            error_msg = f"Failed to retrieve events: {res.get('message', 'Unknown error')}"
            if verbose:
                print(f'  ERROR: {error_msg}', flush=True)
            u.log(api_token, "Maneuvers_update.py", "error", "retrieve events", error_msg)
    except Exception as e:
        print('computeStats exception: '+str(e), flush=True)
        u.log(api_token, "Maneuvers_update.py", "error", "computeStats", str(e))
...
def start(api_token, project_id, class_name, dates, source_names, event_types, channels, start_time, end_time, verbose):
    ...
    Args:
        ...
        event_types: List of event types to compute stats for (e.g., ['TACK', 'GYBE'])
    ...
    try:
        if len(api_token) > 0 and int(project_id) > 0 and len(class_name) > 0:
            u.log(api_token, "Maneuvers_update.py", "info", "starting", "Processing multiple date/source combinations...")
            ...
            for date in dates:
                for source_name in source_names:
                    ...
                    try:
                        ...
                        if res.get("success") and res.get("data"):
                            ...
                            if dataset_id is None:
                                ...
                                u.log(api_token, "Maneuvers_update.py", "warning", "no dataset_id", f"No dataset_id for {date}/{source_name}")
                                continue
                            ...
                            u.log(api_token, "Maneuvers_update.py", "error", "api error", f"Failed to get dataset_id for {date}/{source_name}")
                            continue
                    except Exception as e:
                        if verbose:
                            print(f'Exception retrieving dataset_id for {date}/{source_name}: {str(e)}', flush=True)
                        u.log(api_token, "Maneuvers_update.py", "error", "dataset_id exception", f"{date}/{source_name}: {str(e)}")
                        continue
                    ...
                    if df is None or len(df) == 0:
                        ...
                        u.log(api_token, "Maneuvers_update.py", "warning", "no data", f"No data for {date}/{source_name}")
                        continue

                    u.log(api_token, "Maneuvers_update.py", "info", "data retrieved", f"{date}/{source_name}: {len(df)} records found")
                    ...
                    with ThreadPoolExecutor(max_workers=min(4, len(event_types))) as executor:
                        ...
                        for future in as_completed(futures):
                            try:
                                ...
                                u.log(api_token, "Maneuvers_update.py", "error", "parallel execution", str(e))
            ...
            u.log(api_token, "Maneuvers_update.py", "info", "Maneuver Data Updated!", "Success!")

            if verbose:
                print('\n✓ Maneuver data update completed successfully!', flush=True)
            ...
            print(f"ERROR: {error_msg}", flush=True)
            u.log(api_token, "Maneuvers_update.py", "error", "Invalid parameters", error_msg)
            return False
    except Exception as e:
        import traceback
        error_details = traceback.format_exc()
        print(f"ERROR in Maneuvers_update.start(): {str(e)}", flush=True)
        print(f"Full traceback:\n{error_details}", flush=True)
        u.log(api_token, "Maneuvers_update.py", "error", "Maneuver Data Update Failed!", f"{str(e)}\n{error_details}")
        return False


# Example usage:
# To update maneuver statistics for multiple dates and sources:
