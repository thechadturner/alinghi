...
def start(df, api_token, project_id, dataset_id, class_name, date, source_name, verbose, sec_before = 30, sec_after = 20):
    maneuver_list = df.loc[df['Maneuver_type'] == 'A', 'ts']

    if len(maneuver_list) > 0:
        u.log(api_token, "takeoffs.py", "info", "Processing takeoffs...", str(len(maneuver_list))+" maneuvers found")

        # REMOVE EXISTING MANEUVERS
        jsondata = {"class_name": class_name,"project_id": project_id, "dataset_id": dataset_id, "event_types": ["TAKEOFF"]}
        ...
        for maneuver in maneuver_list:
            ...
            try:
                ...
                if verbose: 
                    print('Working on Takeoff:', climax_time, flush=True)
                ...
                if len(dfn) > 0:
                    ...
                    if len(dfi) > 0:
                        bsp_min = abs(dfi['Bsp_kts']).min()
                        twa_avg = abs(dfi['Twa_deg']).mean()
                        rh_max = abs(dfi['RH_lwd_mm']).max() 

                        if twa_avg > 0:
                            wwd_foil_ext = dfi.iloc[-1]['DB_ext_stbd_mm']
                        else:
                            wwd_foil_ext = dfi.iloc[-1]['DB_ext_port_mm']

                        if bsp_min > 10 or wwd_foil_ext > 1200 or rh_max < 500:
                            u.log(api_token, "takeoffs.py", "info", "Takeoff aborted due to bsp_min > 10 and wwd extension > 1200mm and rh_max < 500mm", str(bsp_min))
                            continue
                        ...
                        new_ts = u.updateManeuverTime(dfi, climax_ts, 'TAKEOFF')
                        ...
                        if (entry_time < 0 and exit_time > 0 and dfm['sec'].min() == -sec_before and dfm['sec'].max() == sec_after): 
                            ...
                            heel_max = abs(dfm['Heel_n_deg']).max()
                            ...
                            # GET POP INFO REPLACES EXIT INFO IN TAKEOFF
                            if entry_tack == 'PORT':
                                pop_mask = (dfm['sec'] > -5) & (dfm['sec'] < 20) & (dfm['RH_stbd_mm'] >= 500)
                            else:
                                pop_mask = (dfm['sec'] > -5) & (dfm['sec'] < 20) & (dfm['RH_port_mm'] >= 500)

                            dfa_pop = dfm.loc[pop_mask].copy()
                            if not dfa_pop.empty:
                                pop_time = dfa_pop.iloc[0]['sec']
                                pop_bsp = dfa_pop.iloc[0]['Bsp_kts']
                                pop_twa = dfa_pop.iloc[0]['Twa_n_deg']
                            else:
                                pop_time = -999
                                pop_bsp = -999
                                pop_twa = -999
                            ...
                            # CALC BS MIN
                            bs_min = dfm.loc[(dfm['sec'] > -30) & (dfm['sec'] < 10), 'Bsp_kts'].min()
                            bs_min_df = dfm.loc[(dfm['Bsp_kts'] == bs_min) & (dfm['sec'] > -30) & (dfm['sec'] < 10), 'sec']
                            ...
                            # CALC ACCEL INFO
                            dfa_after_bs_min = dfm.loc[(dfm['sec'] > bs_min_time) & (dfm['sec'] < pop_time)]
                            ...
                            # CALC TURN ANGLE INFO
                            accel_time = pop_time - bs_min_time
                            ...
                            first_fifteen_seconds = dfi.loc[(dfi['sec'] <= -5)].copy()
                            vmg_start_max = first_fifteen_seconds['Vmg_n_kts'].max() 

                            last_fifteen_seconds = dfi.loc[(dfi['sec'] >= 15)].copy()
                            ...
                            if (mnvr_grade > 0):
                                ...
                                if entry_foiling_state == 'H1' and final_foiling_state == 'H1':
                                    ...
                                    foiling_state =  'H2'
                                elif entry_foiling_state == 'H2' and final_foiling_state == 'H0':
                                    foiling_state = 'H2-H0'
                                elif entry_foiling_state == 'H1'  and final_foiling_state == 'H0':
                                    foiling_state = 'H1-H0'
                                elif entry_foiling_state == 'H2'  and final_foiling_state == 'H1':
                                    foiling_state = 'H2-H1'
                                elif entry_foiling_state == 'H0'  and final_foiling_state == 'H0':
                                    foiling_state =  'H0'
                                elif entry_foiling_state != 'H2' and final_foiling_state == 'H1':
                                ...
                                if (res['success']):
                                    print('Added Takeoff:', u.dt.now(), flush=True)
                                    ...
                                    if event_id > 0:
                                        ...
                                        info["Exit_time"] = round(pop_time, 1)
                                        info["Bsp_exit"] = round(pop_bsp * 1.852, 2) 
                                        info["Twa_exit"] = round(pop_twa, 2)
                                        ...
                                        with ThreadPoolExecutor(max_workers=8) as executor:
                                            ...
                                            # Map data tasks
                                            futures.append(executor.submit(addMapData, dfm, event_id, '0', climax_time, -15, 20, twd_avg, api_token, class_name, project_id))
                                            ...
                                            futures.append(executor.submit(addMapData, dfm, event_id, '3', climax_time, 5, 20, twd_avg, api_token, class_name, project_id))
                                            ...
                                            for future in as_completed(futures):
                                                try:
                                                    ...
                                                    u.log(api_token, "takeoffs.py", "error", "Takeoff Failed! Error in parallel execution", e)
                ...
                failed_count += 1
                error_msg = f"Error processing takeoff at {climax_ts}: {str(e)}"
                u.log(api_token, "takeoffs.py", "error", "Takeoff processing failed", error_msg)
                if verbose:
                    print(f"Failed to process takeoff {climax_ts}: {e}", flush=True)
                import traceback
                u.log(api_token, "takeoffs.py", "error", "Takeoff processing traceback", traceback.format_exc())
                continue

        # Log summary
        summary_msg = f"Processed {processed_count} takeoffs successfully, {failed_count} failed out of {len(maneuver_list)} total"
        u.log(api_token, "takeoffs.py", "info", "Takeoff processing summary", summary_msg)
        if verbose:
        ...
        # No tacks found
        u.log(api_token, "takeoffs.py", "info", "No takeoffs found", "No takeoff maneuvers in data")
        if verbose:
            print("No takeoffs found", flush=True)
        return True
...
def addMapData(df, event_id, desc, mnvr_time, start_sec, stop_sec, twd, api_token, class_name, project_id):
    ...
    if dff.empty:
        u.log(api_token, "takeoffs.py", "warn", "addMapData", f"No data found for event_id {event_id} in time range {start_ts} to {end_ts}")
        return
    ...
    if (res['success'] == False):
        u.log(api_token, "takeoffs.py", "warning", "Map Type "+desc+" Failed!", "")
...
def addTimeSeriesData(type, df, event_id, desc, mnvr_time, start_sec, stop_sec, api_token, class_name, project_id):
    ...
    if type == 'Basics':
        charts = ['tws_kph', 'bsp_kph', 'cwa_n_deg', 'vmg_perc', 'rh_lwd_mm', 'heel_n_deg', 'pitch_deg', 'yaw_rate_n_dps', 'accel_rate_mps2', 'total_turn_angle_deg', 'vmg_loss_m', 'vmg_loss_tgt_m', 'mmg_m']
    elif type == 'Foils':
        charts = ['tws_kph', 'bsp_kph', 'rh_lwd_mm', 'rud_rake_deg', 'db_rake_lwd_deg', 'rud_ang_deg', 'rud_diff_deg', 'db_ext_lwd_mm', 'db_ext_wwd_mm', 'db_cant_lwd_deg']
    elif type == 'Aero':
        charts = ['tws_kph', 'bsp_kph', 'rh_lwd_mm', 'camber1_deg', 'camber6_deg', 'total_twist_deg', 'wing_rotation_deg', 'wing_aoa_deg', 'clew_position_mm', 'jib_sheet_pct', 'jib_sheet_load_kgf', 'jib_cunno_load_kgf', 'jib_lead_ang_deg', 'rig_load_tf']
    ...
    if len(output_str) > 50: 
        ...
        if (res['success'] == False):
            u.log(api_token, "takeoffs.py", "warning", "Time Series "+desc+" Failed!", "")
