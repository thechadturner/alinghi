...
def get_data(api_token, project_id, class_name, date, source_name, start_ts, end_ts):
    ...
    try:
        ...
        u.log(api_token, "tacks.py", "error", "Error occurred while retrieving data!", e)
        return df
...
def start(df, api_token, project_id, dataset_id, class_name, date, source_name, verbose, sec_before = 15, sec_after = 20):
    maneuver_list = df.loc[df['Maneuver_type'] == 'T', 'ts']

    if len(maneuver_list) > 0:
        u.log(api_token, "tacks.py", "info", "Processing tacks...", str(len(maneuver_list))+" maneuvers found")

        # REMOVE EXISTING MANEUVERS
        jsondata = {"class_name": class_name,"project_id": project_id, "dataset_id": dataset_id, "event_types": ["TACK"]}
        ...
        for maneuver in maneuver_list:
            ...
            try:
                ...
                if verbose: 
                    print('Working on Tack:', climax_time, flush=True)
                ...
                if len(dfn) > 0:
                    ...
                    if len(dfi) > 0:
                        ...
                        # PREPARE DATA
                        u.PrepareManeuverData(dfi, 'TACK') 

                        # UPDATE PRFX MANEUVER CLIMAX TIME
                        new_ts = u.updateManeuverTime(dfi, climax_ts, 'TACK')
                        ...
                        if bs_max > 10:
                            entry_time, exit_time = u.IdentifyEntryExit(dfm, -10, 20)
                        else:
                            entry_time, exit_time = [-10, 20]
                        ...
                        if (entry_time < 0 and exit_time > 0 and dfm['sec'].min() == -sec_before and dfm['sec'].max() == sec_after): 
                            ...
                            twd_cor = u.mean360(list(dfm['Twd_cor']))
                            ...
                            # GET DROP INFO
                            drop_mask = (dfm['sec'] > -15) & (dfm['sec'] < 5)
                            dfa_pre_state = dfm.loc[drop_mask].copy()

                            # sort so earliest time is first
                            dfa_pre_state = dfa_pre_state.sort_values('sec')

                            col = 'DB_stow_state_stbd' if entry_tack == 'STBD' else 'DB_stow_state_port'

                            # THIS is the real pre-event state
                            val = dfa_pre_state.iloc[0][col]
                            ...
                            # filter for all rows where the state is still the prestate
                            dfa_drop = dfa_pre_state.loc[dfa_pre_state[col] != prestate]

                            if not dfa_drop.empty:
                                first = dfa_drop.iloc[0]

                                if entry_tack == 'STBD':
                                    drop_cant = first['DB_cant_stbd_deg']
                                    drop_rake = first['DB_rake_ang_stbd_deg']
                                    drop_aoa = first['DB_rake_aoa_stbd_deg']
                                else:
                                    drop_cant = first['DB_cant_port_deg']
                                    drop_rake = first['DB_rake_ang_port_deg']
                                    drop_aoa = first['DB_rake_aoa_port_deg']

                                drop_time = first['sec']
                                drop_bs = first['Bsp_kts']
                                drop_twa = first['Twa_n_deg']
                                drop_pitch = first['Pitch_deg']
                                drop_heel = first['Heel_n_deg']

                                if drop_twa < 90:
                                    drop_cant_tgt = first['ANGLE_CANT_DROP_TARG_UW_deg']
                                else:
                                    drop_cant_tgt = first['ANGLE_CANT_DROP_TARG_DW_deg']
                            else:
                                dfa_drop = dfa_pre_state.loc[drop_mask & (dfa_pre_state[col] >= 100)].copy()
                            ...
                            # CALC TIME DROPPING
                            col2 = 'DB_ext_stbd_mm' if entry_tack == 'STBD' else 'DB_ext_port_mm'

                            drop_mask = (dfm['sec'] > 0) & (dfm[col2] >= 1850)
                            dfa_drop_state = dfm.loc[drop_mask]

                            if not dfa_drop_state.empty:
                                first = dfa_drop_state.iloc[0]
                                dropped_time = first['sec']
                                heel_lock = first['Heel_n_deg']
                                time_dropping  = dropped_time - drop_time
                            else:
                                heel_lock = -999
                                time_dropping = -999
                            ...
                            # GET RAISE INFO
                            raise_mask = (dfm['sec'] > -5) & (dfm['sec'] < 15)
                            dfa_pre_state = dfm.loc[raise_mask].copy()

                            # sort so earliest time is first
                            dfa_pre_state = dfa_pre_state.sort_values('sec')

                            # choose the correct column
                            col = 'DB_stow_state_port' if entry_tack == 'STBD' else 'DB_stow_state_stbd'

                            # 1. true pre-event state = earliest timestamp in window
                            val = dfa_pre_state.iloc[0][col]
                            ...
                            # 2. filter for rows where the state is still the prestate
                            dfa_raise = dfa_pre_state.loc[
                                dfa_pre_state[col] != prestate
                            ].copy()

                            # 3. last moment before the state changes
                            if not dfa_raise.empty:
                                first = dfa_raise.iloc[0]
                                raise_time = first['sec']
                                raise_bs = first['Bsp_kts']
                                raise_pitch = first['Pitch_deg']
                                raise_heel = first['Heel_n_deg']

                                if entry_tack == 'STBD':
                                    raise_aoa = first['DB_rake_aoa_port_deg']
                                    raise_rake = first['DB_rake_ang_port_deg']
                                else:
                                    raise_aoa = first['DB_rake_aoa_stbd_deg']
                                    raise_rake = first['DB_rake_ang_stbd_deg']
                            else:
                                raise_time = -999
                                raise_pitch = -999
                                raise_heel = -999
                            ...
                            # CALC TIME RAISING
                            dfa_raised = dfa_pre_state.loc[
                                dfa_pre_state[col] <= 1
                            ].copy()


                            if len(dfa_raised) >= 1:
                                raised_time = dfa_raised['sec'].iloc[0]   # first raising event
                                time_raising = raised_time - raise_time
                            else:
                                time_raising = -999

                            # GET POP INFO
                            if entry_tack == 'PORT':
                                dfm['WING_twist_deg'] *= -1

                            pop_mask = (dfm['sec'] > -5) & (dfm['sec'] < 15) & (dfm['WING_twist_deg'] <= 0)
                            dfa_pop = dfm.loc[pop_mask].copy()
                            if not dfa_pop.empty:
                                pop_time = dfa_pop.iloc[0]['sec']
                            else:
                                pop_time = -999
                            ...
                            if entry_tack == 'STBD':
                                min_rake = dfa['DB_rake_ang_port_deg'].min()
                                max_rake = dfa['DB_rake_ang_stbd_deg'].max()
                            else:
                                min_rake = dfa['DB_rake_ang_stbd_deg'].min()
                                max_rake = dfa['DB_rake_ang_port_deg'].max()
                            ...
                            # COMPUTE SLOPES
                            if abs(bs_min - entry_bs) > 0 and abs(bs_min_time - entry_time) > 0:             
                                decel_slope = (bs_min - entry_bs) / (bs_min_time - entry_time)
                            else:
                                decel_slope = -999
                            ...
                            mnvr_mask = (dfi['Twa_n_deg'] >= 30) & (dfi['Twa_n_deg'] <= 90)
                            first_fifteen_seconds = dfi.loc[mnvr_mask & (dfi['sec'] <= -5)].copy()
                            vmg_start_max = first_fifteen_seconds['Vmg_n_kts'].max() 

                            last_fifteen_seconds = dfi.loc[mnvr_mask & (dfi['sec'] >= 15)].copy()
                            vmg_end_max = last_fifteen_seconds['Vmg_n_kts'].max()
                            
                            vmg_baseline = max(vmg_start_max, vmg_end_max)
                            dfm['vmg_baseline_kts'] = vmg_baseline
                            dfi['vmg_baseline_kts'] = vmg_baseline

                            #CALC TOTAL LOSS
                            dfc = dfi.loc[(dfi['sec'] >= -15) & (dfi['sec'] <= 20)].copy()
                            vmg_perc_avg = dfc['Vmg_perc'].mean()
                            vmg_avg = dfc['Vmg_n_kts'].mean()
                            mmg = (vmg_avg * u.mps) * 35
                        
                            vmg_total_avg, loss_total_vmg, loss_total_tgt = computeLoss(dfc, 35)
                            ...
                            vmg_build_avg, loss_build_vmg, loss_build_tgt = computeLoss(dfc, 15)

                            # DETERMINE TYPE
                            event_type = 'TACK'
                            mnvr_grade = 3

                            if (exit_tack != end_tack) or (entry_tack == end_tack):
                                mnvr_grade = 1
                            elif (abs(twd_delta) > 15 or max_turn_angle < 30 or max_turn_angle > 150 or abs(init_twa) > 90 or abs(init_twa) < 20 or abs(end_twa) < 20 or abs(end_twa) > 90 or (start_bs > 15 and final_bs < 15)):
                                ...
                                mnvr_grade = 1
                            elif (bs_min < 18 and max_turn_angle > 120) or abs(twd_delta) > 10 or abs(entry_twa) > 65 or abs(exit_twa) > 65 or start_Vmg_perc < 70 or final_Vmg_perc < 70:
                                mnvr_grade = 2
                            ...
                            if (mnvr_grade > 0):
                                ...
                                if (res['success']):
                                    print('Added Tack:', u.dt.now(), flush=True)
                                    ...
                                    if event_id > 0:
                                        ...
                                        info["Time_raising"] = round(time_raising, 2)
                                        info["Time_dropping"] = round(time_dropping, 2)
                                        info["Time_two_boards"] = round(time_two_boards, 2)

                                        info["Accel_max"] = round(accel_max, 2)
                                        info["Accel_min"] = round(accel_min, 2)
                                        info["Decel_slope"] = round(decel_slope, 2)
                                        ...
                                        info["Accel_min_time"] = round(accel_min_time, 1)
                                        ...
                                        info["Drop_time"] = round(drop_time, 2)
                                        info["Bsp_drop"] = round(drop_bs * 1.852, 2)
                                        info["Cant_drop"] = round(drop_cant, 2)
                                        info["Cant_drop_tgt"] = round(drop_cant_tgt, 2)
                                        info["Rake_drop"] = round(drop_rake, 2)
                                        info["Aoa_drop"] = round(drop_aoa, 2)
                                        info["Twa_drop"] = round(drop_twa, 2)
                                        info["Pitch_drop"] = round(drop_pitch, 2)
                                        info["Heel_drop"] = round(drop_heel, 2)
                                        info["Heel_lock"] = round(heel_lock, 2)

                                        info["Raise_time"] = round(raise_time, 2)
                                        info["Bsp_raise"] = round(raise_bs * 1.852, 2)
                                        info["Pitch_raise"] = round(raise_pitch, 2)
                                        info["Heel_raise"] = round(raise_heel, 2)
                                        info["Rake_raise"] = round(raise_rake, 2)
                                        info["Aoa_raise"] = round(raise_aoa, 2)

                                        info["Pop_time"] = round(pop_time, 2)

                                        info["Final_time"] = round(final_time, 2)
                                        info["Bsp_final"] = round(final_bs * 1.852, 2) 
                                        info["Twa_final"] = round(final_twa, 2)

                                        info["Rake_min_old_turn"] = round(min_rake, 2)
                                        info["Rake_max_new_turn"] = round(max_rake, 2)
                                        ...
                                        with ThreadPoolExecutor(max_workers=8) as executor:
                                            ...
                                            # Map data tasks
                                            futures.append(executor.submit(addMapData, dfm, event_id, '0', climax_time, -15, 20, twd_cor, api_token, class_name, project_id))
                                            futures.append(executor.submit(addMapData, dfm, event_id, '1', climax_time, -15, -5, twd_cor, api_token, class_name, project_id))
                                            futures.append(executor.submit(addMapData, dfm, event_id, '2', climax_time, -5, 5, twd_cor, api_token, class_name, project_id))
                                            futures.append(executor.submit(addMapData, dfm, event_id, '3', climax_time, 5, 20, twd_cor, api_token, class_name, project_id))
                                            
                                            # Time series data tasks
                                            futures.append(executor.submit(addTimeSeriesData, 'Basics', dfm, event_id, '0', climax_time, -15, 20, api_token, class_name, project_id))
                                            ...
                                            futures.append(executor.submit(addTimeSeriesData, 'Basics', dfm, event_id, '3', climax_time, 5, 20, api_token, class_name, project_id))

                                            futures.append(executor.submit(addTimeSeriesData, 'Foils', dfm, event_id, '0', climax_time, -15, 20, api_token, class_name, project_id))
                                            ...
                                            futures.append(executor.submit(addTimeSeriesData, 'Foils', dfm, event_id, '3', climax_time, 5, 20, api_token, class_name, project_id))

                                            futures.append(executor.submit(addTimeSeriesData, 'Aero', dfm, event_id, '0', climax_time, -15, 20, api_token, class_name, project_id))
                                            ...
                                            futures.append(executor.submit(addTimeSeriesData, 'Aero', dfm, event_id, '3', climax_time, 5, 20, api_token, class_name, project_id))
                                            ...
                                            for future in as_completed(futures):
                                                try:
                                                    ...
                                                    u.log(api_token, "tacks.py", "error", "Tacks Failed! Error in parallel execution", e)
                ...
                failed_count += 1
                error_msg = f"Error processing tack at {climax_ts}: {str(e)}"
                u.log(api_token, "tacks.py", "error", "Tack processing failed", error_msg)
                if verbose:
                    print(f"Failed to process tack {climax_ts}: {e}", flush=True)
                import traceback
                u.log(api_token, "tacks.py", "error", "Tack processing traceback", traceback.format_exc())
                continue

        # Log summary
        summary_msg = f"Processed {processed_count} tacks successfully, {failed_count} failed out of {len(maneuver_list)} total"
        u.log(api_token, "tacks.py", "info", "Tack processing summary", summary_msg)
        if verbose:
        ...
        # No tacks found
        u.log(api_token, "tacks.py", "info", "No tacks found", "No tack maneuvers in data")
        if verbose:
            print("No tacks found", flush=True)
        return True
...
def addMapData(df, event_id, desc, mnvr_time, start_sec, stop_sec, twd, api_token, class_name, project_id):
    ...
    if dff.empty:
        u.log(api_token, "tacks.py", "warn", "addMapData", f"No data found for event_id {event_id} in time range {start_ts} to {end_ts}")
        return
...
def addTimeSeriesData(type, df, event_id, desc, mnvr_time, start_sec, stop_sec, api_token, class_name, project_id):
    ...
    for index, row in dfc.iterrows():
        ...
        if (type == 'Basics'):
            ...
            item["rud_imm_old_mm"] = round(float(rud_imm_old), 2)
            item["rud_imm_new_mm"] = round(float(rud_imm_new), 2) 
            item["rh_old_mm"] = round(float(rh_old), 2)
            item["rh_new_mm"] = round(float(rh_new), 2) 
            item["db_ext_old_mm"] = round(float(db_ext_old), 2)  
            item["db_ext_new_mm"] = round(float(db_ext_new), 2)
            item["db_cant_old_deg"] = round(float(db_cant_old), 2)
            item["db_cant_new_deg"] = round(float(db_cant_new), 2)
            item["db_rake_old_deg"] = round(float(db_rake_old), 2)
            item["db_rake_new_deg"] = round(float(db_rake_new), 2)
            item["db_rake_aoa_old_deg"] = round(float(db_rake_aoa_old), 2)
            item["db_rake_aoa_new_deg"] = round(float(db_rake_aoa_new), 2)
            item["db_stow_old"] = round(float(db_stow_old), 2)
            item["db_stow_new"] = round(float(db_stow_new), 2)
    ...
    if type == 'Basics':
        charts = ['tws_kph', 'bsp_kph', 'twa_n_deg', 'vmg_perc', 'heel_n_deg', 'pitch_deg', 'yaw_rate_n_dps', 'accel_rate_mps2', 'total_turn_angle_deg', 'vmg_loss_m', 'vmg_loss_tgt_m', 'mmg_m']
    elif type == 'Foils':
        charts = ['tws_kph', 'bsp_kph', 'rud_ang_deg', 'rud_rake_deg', 'rud_diff_deg', 'rud_imm_old_mm', 'rud_imm_new_mm', 'rh_old_mm', 'rh_new_mm', 'db_ext_old_mm', 'db_ext_new_mm', 'db_stow_old', 'db_stow_new', 'db_cant_old_deg', 'db_cant_new_deg', 'db_rake_old_deg', 'db_rake_new_deg', 'db_rake_aoa_old_deg', 'db_rake_aoa_new_deg']
    elif type == 'Aero':
