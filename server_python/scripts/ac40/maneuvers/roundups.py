...
def get_data(api_token, project_id, class_name, date, source_name, start_ts, end_ts):
    ...
    try:
        ...
        u.log(api_token, "roundups.py", "error", "Error occurred while retrieving data!", e)
        return df
...
def start(df, api_token, project_id, dataset_id, class_name, date, source_name, verbose, sec_before = 15, sec_after = 15):
    maneuver_list = df.loc[df['Maneuver_type'] == 'R', 'ts']

    if len(maneuver_list) > 0:
        u.log(api_token, "roundups.py", "info", "Processing roundups...",  str(len(maneuver_list))+" maneuvers found")

        # REMOVE EXISTING MANEUVERS
        jsondata = {"class_name": class_name,"project_id": project_id, "dataset_id": dataset_id, "event_types": ["ROUNDUP","UFO","JK"]}
        ...
        for maneuver in maneuver_list:
            ...
            try:
                ...
                if verbose: 
                    print('Working on Roundup:', climax_time, flush=True)
                ...
                if len(dfn) > 0:
                    ...
                    if len(dfi) > 0:
                        ...
                        new_ts = u.updateManeuverTime(dfi, climax_ts, 'ROUNDUP')
                        ...
                        if (entry_time < 0 and exit_time > 0 and dfm['sec'].min() == -sec_before and dfm['sec'].max() == sec_after): 
                            ...
                            dfa = dfm.loc[(dfm['sec'] > 0) & (dfm['sec'] < sec_after) & (abs(dfm['Yaw_rate_dps']) < 2)]
                            ...
                            if init_tack != origin_tack and twa_max > 160 and abs(entry_twa) > 90 and abs(end_twa) > 30 and abs(end_twa) < 90:
                                event_type = 'UFO'
                            elif origin_tack != end_tack and twa_min < 20 and abs(entry_twa) > 90 and abs(end_twa) > 30 and abs(end_twa) < 90:
                                event_type = 'JK'

                            if abs(end_twa) > 90:
                                mnvr_grade = 0
                            elif abs(max_turn_angle) < 10 or (event_type == 'ROUNDUP' and abs(final_turn_cum) > 300) or abs(twd_delta) > 15 or bs_min < 10:
                                ...
                                mnvr_grade = 1
                            elif bs_min < 18 and abs(max_turn_angle) > 100 and abs(max_turn_angle) < 20 or abs(twd_delta) > 10:
                                mnvr_grade = 2
                            ...
                            if (mnvr_grade > 0):
                                ...
                                if (res['success']):
                                    print('Added RoundUp:', u.dt.now(), flush=True)
                                        ...
                                        with ThreadPoolExecutor(max_workers=8) as executor:
                                            ...
                                            for future in as_completed(futures):
                                                try:
                                                    ...
                                                    u.log(api_token, "roundups.py", "error", "Roundups Failed! Error in parallel execution", e)
                ...
                failed_count += 1
                error_msg = f"Error processing roundup at {climax_ts}: {str(e)}"
                u.log(api_token, "roundups.py", "error", "Roundups processing failed", error_msg)
                if verbose:
                    print(f"Failed to process roundup {climax_ts}: {e}", flush=True)
                import traceback
                u.log(api_token, "roundups.py", "error", "Roundups processing traceback", traceback.format_exc())
                continue

        # Log summary
        summary_msg = f"Processed {processed_count} Roundups successfully, {failed_count} failed out of {len(maneuver_list)} total"
        u.log(api_token, "roundups.py", "info", "Roundups processing summary", summary_msg)
        if verbose:
        ...
        # No roundups found
        u.log(api_token, "roundups.py", "info", "No roundups found", "No roundup maneuvers in data")
        if verbose:
            print("No roundups found", flush=True)
        return True
...
def addMapData(df, event_id, desc, mnvr_time, start_sec, stop_sec, twd, api_token, class_name, project_id):
    ...
    if dff.empty:
        u.log(api_token, "roundups.py", "warn", "addMapData", f"No data found for event_id {event_id} in time range {start_ts} to {end_ts}")
        return
    ...
    if desc == '1':
        if len(dff) == 0:
            u.log(api_token, "roundups.py", "warn", "addMapData", f"No data in dff for event_id {event_id}, desc='1'")
            return
        ...
        if len(dfa) == 0:
            u.log(api_token, "roundups.py", "warn", "addMapData", f"No data in dfa for event_id {event_id}, desc='{desc}'")
            return
