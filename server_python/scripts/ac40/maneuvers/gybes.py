...
def get_data(api_token, project_id, class_name, date, source_name, start_ts, end_ts):
    ...
    try:
        ...
        u.log(api_token, "gybes.py", "error", "Error occurred while retrieving data!", e)
        return df
...
def start(df, api_token, project_id, dataset_id, class_name, date, source_name, verbose, sec_before = 15, sec_after = 20):
    maneuver_list = df.loc[df['Maneuver_type'] == 'G', 'ts']

    if len(maneuver_list) > 0:
        u.log(api_token, "gybes.py", "info", "Processing gybes...",  str(len(maneuver_list))+" maneuvers found")

        # REMOVE EXISTING MANEUVERS
        jsondata = {"class_name": class_name,"project_id": project_id, "dataset_id": dataset_id, "event_types": ["GYBE"]}
        ...
        for maneuver in maneuver_list:
            ...
            try:
                ...
                if verbose: 
                    print('Working on Gybe:', climax_time, flush=True)
                ...
                if len(dfn) > 0:
                    ...
                    if len(dfi) > 0:
                        ...
                        # PREPARE DATA
                        u.PrepareManeuverData(dfi, 'GYBE') 

                        # UPDATE PRFX MANEUVER CLIMAX TIME
                        new_ts = u.updateManeuverTime(dfi, climax_ts, 'GYBE')
                        ...
                        if (entry_time < 0 and exit_time > 0 and dfm['sec'].min() == -sec_before and dfm['sec'].max() == sec_after): 
                            ...
                            mnvr_mask = (dfi['Twa_n_deg'] >= 90) & (dfi['Twa_n_deg'] <= 155)
                            ...
                            if (exit_tack != end_tack) or (entry_tack == end_tack):
                                mnvr_grade = 1
                            elif (abs(twd_delta) > 15 or max_turn_angle < 30 or max_turn_angle > 150 or abs(init_twa) < 90 or abs(init_twa) > 165 or abs(end_twa) > 165 or abs(end_twa) < 90 or (start_bs > 15 and final_bs < 15)):
                                ...
                                mnvr_grade = 1
                            elif (bs_min < 18 and max_turn_angle > 120) or abs(twd_delta) > 10 or start_Vmg_perc < 70 or final_Vmg_perc < 70:
                                mnvr_grade = 2
                            ...
                            if (mnvr_grade > 0):
                                ...
                                if (res['success']):
                                    print('Added Gybe:', u.dt.now(), flush=True)
                                        ...
                                        with ThreadPoolExecutor(max_workers=8) as executor:
                                            ...
                                            for future in as_completed(futures):
                                                try:
                                                    ...
                                                    u.log(api_token, "gybes.py", "error", "Gybes Failed! Error in parallel execution", e)
                ...
                u.log(api_token, "gybes.py", "error", "Gybe processing failed", error_msg)
                if verbose:
                    print(f"Failed to process tack {climax_ts}: {e}", flush=True)
                import traceback
                u.log(api_token, "gybes.py", "error", "Gybe processing traceback", traceback.format_exc())
                continue

        # Log summary
        summary_msg = f"Processed {processed_count} gybes successfully, {failed_count} failed out of {len(maneuver_list)} total"
        u.log(api_token, "gybes.py", "info", "Gybe processing summary", summary_msg)
        if verbose:
        ...
        # No gybes found
        u.log(api_token, "gybes.py", "info", "No gybes found", "No gybe maneuvers in data")
        if verbose:
            print("No gybes found", flush=True)
        return True
...
def addMapData(df, event_id, desc, mnvr_time, start_sec, stop_sec, twd, api_token, class_name, project_id):
    ...
    if dff.empty:
        u.log(api_token, "gybes.py", "warn", "addMapData", f"No data found for event_id {event_id} in time range {start_ts} to {end_ts}")
        return
