...
if __name__ == "__main__":
    ...
    # verbose = s.get_item('verbose') or False  # Default to False if not provided or None
    ...
    u.log(api_token, "0_maneuvers.py", "info", f"Executing script on ${source_name} and ${date}", str(u.dt.now()))

    success_status = []

    #Execute Scripts
    import Maneuvers
    success = Maneuvers.start(api_token, project_id, dataset_id, class_name, date, source_name, None, None, verbose)
    success_status.append(success)

    # UPDATE DATASET VISIBILITY
    if any(not status for status in success_status):
        u.log(api_token, "0_maneuvers.py", "info", "Scripts Failed!", str(u.dt.now()))
        ...
        try:
            ...
            u.log(api_token, "0_maneuvers.py", "warn", "date_modified update", f"Failed to update date_modified: {str(e)}")

        u.log(api_token, "0_maneuvers.py", "info", "Script Completed!", str(u.dt.now()))
